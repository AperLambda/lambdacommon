...
{
    Address::Address(const host &host, port port) : _host(new string(host)), _port(port)
    {}
    ...
    string Address::toString() const
    {
        ...
        string lPort = (_port != 0 ? ":" + to_string(_port) : "");
        string lHost = *_host;
        switch (getType())
        {
            case IPv6:
                ...
                break;
            case INVALID:
                lHost = "INVALID_DOMAIN";
                break;
            default:
                break;
        }
        return lHost + (_port != 0 ? ":" + to_string(_port) : "");
    }

    Address &Address::operator=(const Address &address)
    {
        delete _host;
        _host = new host(*address._host);
        _port = address._port;
        return *this;
    }
    ...
    Address &Address::operator=(Address &&address) noexcept
    {
        ...
        {
            delete _host;
            _host = new host(move(*address._host));
            _port = address._port;
        }
        return *this;
    }
    ...
    {
        return *_host == (*address._host) && _port == address._port;
    }
