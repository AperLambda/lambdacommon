...
#  else // __MINGW32__
        ...
        if (FAILED(SHGetKnownFolderPath(FOLDERID_Profile, KF_FLAG_NO_ALIAS, nullptr, &sz_path)))
            return "";
        return std::string(lstring::from_wstring_to_utf8(std::wstring(sz_path)));
#  endif // __MINGW32__
    }
    ...
    std::string LAMBDACOMMON_API get_os_name() {
        ...
        if (!os_name.empty())
            return os_name;
        fs::path etc_os_release{"/etc/os-release"};
        fs::path lsb_release{"/etc/lsb-release"};
        struct utsname uts{};
        uname(&uts);
    ...
    fs::path LAMBDACOMMON_API get_user_directory() {
    ...
    std::string LAMBDACOMMON_API get_error_message(int error_code) {
        ...
        LocalFree(msg_buf);
        return msg;
#elif defined(LAMBDA_MAC_OSX) || defined(LAMBDA_WASM) || defined(LAMBDA_ANDROID) || defined(LAMBDA_BSD)
        char buffer[512];
