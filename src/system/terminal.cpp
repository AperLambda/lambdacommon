...
namespace lambdacommon::terminal
{
...
#define RD_EIO -2
    ...
    Point2D_u16 LAMBDACOMMON_API get_cursor_position(std::ostream& stream) {
        if (is_tty(stream)) {
            ...
            if (GetConsoleScreenBufferInfo(get_term_handle(stream), &csbi))
                return {static_cast<u16>(csbi.dwCursorPosition.X), static_cast<u16>(csbi.dwCursorPosition.Y)};
#else
            ...
            do {
                ...
                while (result >= '0' && result <= '9') {
                    rows = static_cast<u16>(10 * rows + result - '0');
                    result = rd(tty);
                }
                ...
                result = rd(tty);
                while (result >= '0' && result <= '9') {
                    columns = static_cast<u16>(10 * columns + result - '0');
                    result = rd(tty);
                }
                ...
                cursor_pos.set_x(static_cast<u16>(columns - 1));
                cursor_pos.set_y(static_cast<u16>(rows - 1));
                break;
            } while (true);
    ...
    const Size2D_u16 get_size(const std::ostream& stream) {
        ...
        size.set_width(static_cast<u16>(csbi.srWindow.Right - csbi.srWindow.Left + 1));
        size.set_height(static_cast<u16>(csbi.srWindow.Bottom - csbi.srWindow.Top + 1));
#else
        struct winsize w{};
