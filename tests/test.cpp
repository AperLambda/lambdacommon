...
LC_TEST_SECTION(FileSystem)
{
	...
	{
		REQUIRE(fs::get_current_working_directory().exists());
		REQUIRE(!fs::FilePath("404_non_existent").exists());
	}

	LC_TEST(fs_fp_is_dir, "FilePath::is_directory()")
	{
		REQUIRE(fs::get_current_working_directory().is_directory());
	}

	LC_TEST(fs_fp_op, "FilePath / operator")
	{
		REQUIRE((fs::get_current_working_directory() / "not_found_dir").get_filename() == "not_found_dir");
	}
}
...
LC_TEST_SECTION(URI)
{
	LC_TEST(uri_to_string, "URI::to_string()")
	{
		REQUIRE(URI("https",
					...
					{{"foo",               "bar"},
					 ...
					 {"queryWithoutValue", ""}},
					"bar").to_string() == "https://user:pwd@something.com/test/file_or_folder?foo=bar&invalid%20query=but%20fixed&queryWithoutValue#bar");
	}
	...
	{
		auto uri_string = "https://www.youtube.com/channel/UC2i7nj6wnh1Z2GQwvFeeKoA";
